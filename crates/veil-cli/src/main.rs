...
enum Mode {
    SelfHosted { message: String, hop_count: usize },
    External { topology_path: String, message: String },
}
...
fn parse_args() -> Mode {
    ...
    if let Some(flag_pos) = args.iter().position(|a| a == "--topology") {
        ...
        Mode::External { topology_path, message }
    } else {
        let message = args.first().cloned().unwrap_or_else(|| "hello from veil-cli".to_string());
...
async fn main() {
    match parse_args() {
        ...
        Mode::External { topology_path, message } => run_external(topology_path, message).await,
    }
}
...
async fn run_self_hosted(message: String, hop_count: usize) {
    println!("veil-cli: spinning up {hop_count} local relay(s), each with its own mailbox listener");
...
async fn run_external(topology_path: String, message: String) {
    ...
    println!("veil-cli: loaded {} relay(s); routing through {hop_count}-hop circuits", topology.len());
    ...
    let mailbox_addrs: Vec<String> = topology.all().map(|r| mailbox_address_from(&r.address)).collect();
...
async fn send_and_receive(topology: Topology, hop_count: usize, message: String, mailbox_addrs: Vec<String>) {
    ...
    println!("veil-cli: receiving — polling {} relay mailbox(es) over the network", mailbox_addrs.len());
    let received = receiver::receive(&recipient_identity, &mailbox_addrs).await.expect("receive failed");
